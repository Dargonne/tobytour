      ...
      <div class="row">
        <div class="col-xs-12">
          <h2 class="page-header">
            <i class="fa fa-globe"></i> メンバーリストを表示
          </h2>
        </div>
        <!-- /.col -->
      </div>
            ...
            <tbody>
             <c:forEach var="member" items="${memberList }">
				<tr class="record">
					...
					<td><fmt:formatDate value="${member.memberJoinDate }"/></td>
					<td>
						<c:if test="${member.memberUserSection == 0}">
       					 	[ユーザー]
  						</c:if>
  						<c:if test="${member.memberUserSection == 1}">
       					 	[管理者]
  						</c:if>
					</td>
					<td><a href="./MemberServlet?command=member_view&seq=${member.memberID}">詳細を見る</a></td>
				</tr>
			</c:forEach>
            <tr>
					...
					<c:if test="${count > 0}">
     				 ...
     				 <c:if test="${endPage > pageCount}" >
   					...
   					<c:if test="${numPageGroup > 1}">
       					  <li><a href="./MemberServlet?command=member_list&page=${(numPageGroup-2)*pageGroupSize+1 }">[前]</a></li>
  					</c:if>

   					<c:forEach var="i" begin="${startPage}" end="${endPage}">
      					  <li><a href="./MemberServlet?command=member_list&page=${i}">[
  					 ...
  					 <c:if test="${numPageGroup < pageGroupCount}">
        				<li><a href="./MemberServlet?command=member_list&page=${numPageGroup*pageGroupSize+1}">[次]</a></li>
   					</c:if>
				</c:if>
				</ul>
				</div>
