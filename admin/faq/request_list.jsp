        ...
        <div class="col-xs-12 table-responsive">
          <table class="table table-striped">
            ...
            <c:forEach var="request" items="${requestList }">
				<tr class="record">
					<td>${request.requestSeq }</td>
					<td><a href="./RequestServlet?command=request_update_form&seq=${request.requestSeq}">${request.requestTitle }</a></td>
					<td>${request.requestID }</td>
					<td><fmt:formatDate value="${request.requestWriteDate }"/></td>
					<td>
						<c:if test="${request.requestReply == 0}">
				              <a href="./RequestServlet?command=request_update_form&seq=${request.requestSeq}">[回答の登録]</a>
  						</c:if>
  						<c:if test="${request.requestReply == 1}">
							[回答完了]
  						</c:if>
					</td>
				</tr>
					...
					<c:if test="${count > 0}">
     				 ...
     				 <c:if test="${endPage > pageCount}" >
   					...
   					<c:if test="${numPageGroup > 1}">
       					  <li><a href="./RequestServlet?command=request_list&page=${(numPageGroup-2)*pageGroupSize+1 }">[前]</a></li>
  					</c:if>

   					<c:forEach var="i" begin="${startPage}" end="${endPage}">
      					  <li><a href="./RequestServlet?command=request_list&page=${i}">[
  					 ...
  					 <c:if test="${numPageGroup < pageGroupCount}">
        				<li><a href="./RequestServlet?command=request_list&page=${numPageGroup*pageGroupSize+1}">[次]</a></li>
   					</c:if>
				</c:if>
				</ul>
				</div>
