          ...
          <table class="table table-striped">
			...
			<c:forEach var="notice" items="${noticeList}">
				<tr class="record">
					...
					<td><a href="./NoticeServlet?command=notice_view&seq=${notice.noticeSeq}">${notice.noticeTitle }</a></td>
					<td><fmt:formatDate value="${notice.noticeWriteDate }"/></td>
				</tr>
			</c:forEach>
            <tr>
					...
					<c:if test="${count > 0}">
     				 ...
     				 <c:if test="${endPage > pageCount}" >
   					...
   					<c:if test="${numPageGroup > 1}">
       					  <li><a href="./NoticeServlet?command=notice_list&page=${(numPageGroup-2)*pageGroupSize+1 }">[前]</a></li>
  					</c:if>

   					<c:forEach var="i" begin="${startPage}" end="${endPage}">
      					  <li><a href="./NoticeServlet?command=notice_list&page=${i}">[
  					 ...
  					 <c:if test="${numPageGroup < pageGroupCount}">
        				<li><a href="./NoticeServlet?command=notice_list&page=${numPageGroup*pageGroupSize+1}">[次]</a></li>
   					</c:if>
				</c:if>
					<li><a href="./NoticeServlet?command=notice_write_form">お知らせ登録</a></li>
				</ul>
				</div>
				</td>
			</tr>
