...
<head>
  ...
  <script type="text/javascript" src="./js/subproduct.js"></script>
</head>
          ...
          <table class="table table-striped">
            ...
            <tr>
              ...
              <div class="callout callout-info">
        			<h4>注意!</h4>
       				メイン商品について出発日別の商品が登録された場合には、メイン商品の削除ができませんので<br/>
       				慎重に作成していただき、削除が必要な場合は、管理者にお問い合わせください。
      			</div>
              </th>
            </tr>
            </thead>
            <tbody>
            <tr>
				<td rowspan="2" width="250"><img src="${productFilePath }" width="250" height="120" border="0" /></td>
				<td>商品名 : ${mainList.mainProductNo }</td>
			</tr>
			<tr>
				<td>出発曜日 : ${mainList.mainDepDay } </td>
			</tr>           
            </tbody>
            </table>
         </div>
        ...
        <div class="col-xs-12 table-responsive">
          <table class="table table-striped">
           <tbody>
			...
			<tr>
				<td>${mainList.mainIncludeList }</td>
				<td>${mainList.mainExcludelist }</td>
				<td>${mainList.mainChoiceCost }</td>				
			</tr>		
            </tbody>
            </table>
         </div>
      ...
      <div class="row">
        <div class="col-xs-12 table-responsive">
          <table class="table table-striped">
           <tbody>
            <tr>
				<td rowspan="2" width="250"><img src="${hotelFilePath }" width="250" height="120" border="0" /></td>
				<th>ホテル名 : ${mainList.mainHotelName }</th>
			</tr>
			<tr>
				<th>ホテル情報 : ${mainList.mainHotelInfo } </th>
			</tr>	
            </tbody>
            </table>
         </div>
        ...
        <div class="col-xs-12 table-responsive">
          <table class="table table-striped">
           <tbody>
			...
			<c:forEach var="sub" items="${subProductList }">
			<tr>
				<td>${sub.subProductNo } </td>
				<td><fmt:formatDate value="${sub.subStartDay }"/></td>
				<td><fmt:formatDate value="${sub.subEndDay }"/></td>
				<td>${sub.subAirLine }</td>
				<td>${sub.subProductLeast }</td>
				<td><a href="./ProductServlet?command=product_sub_view&seq=${sub.subProductNo}">見る</a></td>				
			</tr>	
			</c:forEach>
			<tr>
					...
					<c:if test="${count > 0}">
     				 ...
     				 <c:if test="${endPage > pageCount}" >
   					...
   					<c:if test="${numPageGroup > 1}">
       					 <a href="./ProductServlet?command=product_main_view&page=${(numPageGroup-2)*pageGroupSize+1 }">[前]</a>
  					</c:if>

   					<c:forEach var="i" begin="${startPage}" end="${endPage}">
      					 <a href="./ProductServlet?command=product_main_view&page=${i}">[
  					 ...
  					 <c:if test="${numPageGroup < pageGroupCount}">
        				<a href="./ProductServlet?command=product_main_view&page=${numPageGroup*pageGroupSize+1}">[次]</a>
   					</c:if>
				</c:if>
				</td>
			</tr>
			<tr>
				<td colspan="6" align="center">
				 <c:if test="${count == 0}">
        				<a href="./ProductServlet?command=product_main_delete&seq=${seqNumber }" class="btn btn-default">削除</a>
        				<a href="./ProductServlet?command=product_list" class="btn btn-default">メイン商品一覧</a>
        				<a href="./ProductServlet?command=product_sub_write&seq=${seqNumber }" onClick="window.open(this.href, '', 'width=520, height=320'); return false;" class="btn btn-default">日別商品登録</a>
   				 </c:if>
   				 <c:if test="${count != 0 }">
        				<a href="./ProductServlet?command=product_list" class="btn btn-default">メイン商品一覧</a>
        				<a href="./ProductServlet?command=product_sub_write&seq=${seqNumber }" onClick="window.open(this.href, '', 'width=520, height=320'); return false;" class="btn btn-default">日別商品登録</a>
