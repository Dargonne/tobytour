        ...
        <div class="col-xs-12 table-responsive">
          <table class="table table-striped">
			...
			<c:forEach var="product" items="${productList}">
				<tr class="record">
					<td>${product.mainproduct_no}</td>
					<td>${product.productname}</td>
					<td style="text-align: right"><input type="button"  class="btn btn-default"  value="情報を表示する" 
					onclick='location.href="./ProductServlet?command=product_main_view&seq=${product.mainproduct_no}"'></td>
				</tr>
			</c:forEach>
			<tr>
					...
					<c:if test="${count > 0}">
     				 ...
     				 <c:if test="${endPage > pageCount}" >
   					...
   					<c:if test="${numPageGroup > 1}">
       					  <li><a href="./ProductServlet?command=product_list&page=${(numPageGroup-2)*pageGroupSize+1 }">[前]</a></li>
  					</c:if>

   					<c:forEach var="i" begin="${startPage}" end="${endPage}">
      					  <li><a href="./ProductServlet?command=product_list&page=${i}">[
  					 ...
  					 <c:if test="${numPageGroup < pageGroupCount}">
        				<li><a href="./ProductServlet?command=product_list&page=${numPageGroup*pageGroupSize+1}">[次]</a></li>
   					</c:if>
				</c:if>
				</ul>
				</div>
