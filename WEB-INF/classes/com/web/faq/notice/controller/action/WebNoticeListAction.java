...
public class WebNoticeListAction implements WebNoticeAction{

	@Override
	public void execute(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		String url = "/web/faq/notice/notice_list.jsp";
	   ...
	   // ArrayList articleList = new ArrayList(); // Paging 처리를 위한 ArrayList
		...
		List<WebNoticeDTO> noticeList = new ArrayList(); 

		count = dao.getNoticeTotal();//전체 글의 수
		 if (count > 0) {        
			 if(endRow>count)
				 endRow = count;
			     noticeList = dao.showAllNoticeList(startRow, endRow);
